{
  ...
  outputs = inputs@{ self, ... }: inputs.functions.lib.importRepo inputs ./. (repo@{ ... }:
    let
      lib = repo.lib.__internal__;
    in
    [
      # Run »nix flake show --allow-import-from-derivation« to see what this merges to:
      repo # lib.* nixosModules.* overlays.* packages.*.*  patches.*
      (inputs.installer.lib.mkSystemsFlake { inherit inputs; }) # nixosConfigurations.* apps.*-linux.* devShells.*-linux.* packages.*-linux.all-systems
    ]);
}
